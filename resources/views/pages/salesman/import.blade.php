<x-app-layout title="Import Salesman">
    <x-card title="Import Salesman">
        <form action="{{ route('salesman.import.preview') }}" method="POST" enctype="multipart/form-data">
            @csrf
            ...
            <div class="flex gap-2">
                ...
                <a href="{{ route('salesman.import.template') }}">
    ...
    @isset($rows)
        <x-card title="Preview Data" class="mt-3" desc="{{ $rows->count() }} data ditemukan.">
            ...
            @if ($rows->isNotEmpty())
                <div class="mt-4">
                    <x-form action="{{ route('salesman.import.store') }}" method="POST">
